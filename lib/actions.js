  ...
  const getAll = (url, {
    ...
    try {
      ...
      if (!response.status) {
        ...
        dispatchSuccess(
          dispatch, response.status, data.data, data.permissions || {}, SUCCESS, { query }
        );
      } else if (response.status >= 400 && response.status < 500) {
        const data = await response.json();

        dispatchErrors(dispatch, response, FAILURE, { query, error: data.error_code });
      } else {
  ...
  const get = (url, {
    ...
    try {
      ...
      if (!response.status) {
        ...
        dispatchSuccess(
          dispatch, response.status, data.data, data.permissions || {}, SUCCESS, { id, query }
        );
      } else if (response.status >= 400 && response.status < 500) {
        const data = await response.json();

        dispatchErrors(dispatch, response, FAILURE, { id, query, error: data.error_code });
      } else {
  ...
  const put = (url, {
    ...
    try {
      ...
      if (!response.status) {
        ...
        dispatchSuccess(dispatch, response.status, data.data, data.permissions || {}, SUCCESS, { id });
      } else if (response.status >= 400 && response.status < 500) {
        const data = await response.json();

        dispatchErrors(dispatch, response, FAILURE, { id, error: data.error_code });
      } else {
  ...
  const patch = (url, {
    ...
    try {
      ...
      if (!response.status) {
        ...
        dispatchSuccess(dispatch, response.status, data.data, data.permissions || {}, SUCCESS, { id, patchId });
      } else if (response.status >= 400 && response.status < 500) {
        const data = await response.json();

        dispatchErrors(dispatch, response, FAILURE, { id, patchId, error: data.error_code });
      } else {
  ...
  const removeAll = (url, {
    ...
    try {
      ...
      if (!response.status) {
        ...
        dispatchErrors(dispatch, response, FAILURE, { ids, id, error: data.error_code });
      } else {
